...
for f = 2.45e9%[0.5e9, 1.5e9, 2.5e9]
    ...
    rad_intensity_ground_0 = r.^2 .* (1/1024).*epsilon.^(-1).*I0.^2.*k.^2.*l.^2.*omega.^(-1).*pi.^(-2) ...
        .*r.^(-5).*(r.^2).^(-1/2).*(2048.*k.*l.^2.*r.^2+24.*k.*r.^4+266.* ...
        k.^3.*l.^2.*r.^4+(sqrt(-1)*(-2048)).*l.^2.*(r.^2).^(1/2)+(-8).*k.* ...
        r.^2.*((-1).*r.^2+16.*l.^2.*(2+k.^2.*r.^2)).*cos(2.*phi)+6.*k.^3.* ...
        l.^2.*r.^4.*cos(4.*phi)+(-4).*k.^3.*l.^2.*r.^4.*cos(4.*phi+(-2).* ...
        theta)+128.*k.*l.^2.*r.^2.*cos(2.*(phi+(-1).*theta))+(-4).*k.* ...
        r.^4.*cos(2.*(phi+(-1).*theta))+64.*k.^3.*l.^2.*r.^4.*cos(2.*(phi+ ...
        (-1).*theta))+k.^3.*l.^2.*r.^4.*cos(4.*(phi+(-1).*theta))+(-2048) ...
        .*k.*l.^2.*r.^2.*cos(2.*theta)+8.*k.*r.^4.*cos(2.*theta)+(-264).* ...
        k.^3.*l.^2.*r.^4.*cos(2.*theta)+(sqrt(-1)*2048).*l.^2.*(r.^2).^( ...
        1/2).*cos(2.*theta)+(-2).*k.^3.*l.^2.*r.^4.*cos(4.*theta)+128.*k.* ...
        l.^2.*r.^2.*cos(2.*(phi+theta))+(-4).*k.*r.^4.*cos(2.*(phi+theta)) ...
        +64.*k.^3.*l.^2.*r.^4.*cos(2.*(phi+theta))+k.^3.*l.^2.*r.^4.*cos( ...
        4.*(phi+theta))+(-4).*k.^3.*l.^2.*r.^4.*cos(2.*(2.*phi+theta))+( ...
        -256).*k.*l.*r.^3.*sin(phi+(-1).*theta)+(sqrt(-1)*256).*l.*r.*( ...
        r.^2).^(1/2).*sin(phi+(-1).*theta)+256.*k.*l.*r.^3.*sin(phi+theta) ...
        +(sqrt(-1)*(-256)).*l.*r.*(r.^2).^(1/2).*sin(phi+theta)).*sin(h.* ...
        k.*cos(theta)).^2;
    ...
    if plot_sim
        ...
        if sim_withsub
            ...
            RF_surface_0 = csvread(sprintf('11segloop2_%.2fG_E_final.csv', f/1e9), 1);
        ...
        if sim_withsub
            ...
            RF_surface_90 = csvread(sprintf('11segloop2_%.2fG_H_final.csv', f/1e9), 1);

...
gain = [0.2 -2.9 -7.7;      %REF
       -11.2 -16.2 -12.5;       %NIP
       2 -0.8 0];         %IP
   
efficiency = [0.277 0.146 0.055;
              0.016 0.006 0.012;
              0.432 0.195 0.261];
...
for i = 1:length(antenna_type)
    text(antenna_type(i) - 0.3, gain_freq1(i, 1), num2str(gain_freq1(i, 1) + gain_offset,'%0.1f'),...
...
hold on
for i = 1:length(antenna_type)
    text(antenna_type(i) - 0.3, gain_freq2(i, 1), num2str(gain_freq2(i, 1) + gain_offset,'%0.1f'),...
        'vert','bottom', 'color', [0.8500    0.3250    0.0980], 'Fontsize', fontsize, 'fontweight', 'bold');
end
ylabel('Peak Gain (dBi)');
...
hold on
for i = 1:length(antenna_type)
    text(antenna_type(i) - 0.3, gain_freq3(i, 1), num2str(gain_freq3(i, 1) + gain_offset,'%0.1f'),...
        'vert','bottom', 'color', [0.8500    0.3250    0.0980], 'Fontsize', fontsize, 'fontweight', 'bold');
end
ylabel('Peak Gain (dBi)');

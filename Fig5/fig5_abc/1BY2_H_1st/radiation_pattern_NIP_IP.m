...
antenna_type = '1BY2';
xz_plane_plot_enable = 1;  % H
yz_plane_plot_enable = 0;  % E
...
if xz_plane_plot_enable == 1
    for k = 1:length(tar_freq_NIP)   % how many frequencies
        ...
        %'TitleTop', sprintf('%s H-plane radiation plot at TM10 mode', antenna_type), 
        hold on
...
if yz_plane_plot_enable == 1
    ...
    for k = 1:length(tar_freq_NIP)   % how many frequencies
        ...
        polarpattern(ang_vertical, G_vertical_db_IP(:, k), 'AngleDirection','CW', 'ConnectEndpoints', 1, 'AngleResolution', 10, 'ColorOrder', line_color, 'LineWidth', line_width, 'LineStyle', line_style, ...
            'AngleAtTop', 0, 'MagnitudeAxisAngle', 0, 'MagnitudeLim', [-40 0]);
        hold on
...
if simulation_plot_enable == 1
    for k = 1:length(tar_freq_NIP)   % how many frequencies
        ...
        if xz_plane_plot_enable == 1
            ...
            data = csvread([antenna_type, '_NIP_65mm_freq', int2str(k) ,'_xz' , '.csv'], 1, 0);  % skips the first row of data 
            ...
            data = csvread([antenna_type, '_IP_65mm_freq', int2str(k) ,'_xz' , '.csv'], 1, 0);  % skips the first row of data 
        ...
        if yz_plane_plot_enable == 1
            ...
            legend({'Measured NIP', 'Measured IP', 'Simulated NIP', 'Simulatied IP'}, 'FontSize', 12, 'position', [0.04 0.12 0.1 0.05])%[0.58 0.27 0.1 0.05]);
        end
    end
end

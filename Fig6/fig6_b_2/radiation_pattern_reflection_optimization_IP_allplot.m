...
line_color = [0 0 0; 1 0 0; 1 0.65 0.65; 1 0.9 0.9];
line_style = {':', '-', '-', '-', '-', '-', '-'};
...
for k = plot_group
    ...
    if xz_plane_plot_enable == 1
        ...
        for j = 1:length(tar_freq)
            ...
            %'TitleTop', {'IP FPRFS emulated 1by2-2-1 patch antenna with ', sprintf('     environmental obstacles at %.2fGHz', 2.22)}, 
            hold on;                                                                                                                                    %tar_freq(j)/1e9) 
            %polarpattern(ang(37), G_db(37, j), 'Marker', marker, 'ColorOrder', line_color, 'LineWidth', line_width);

            if ~simulation_plot_enable
                if plot_group(2) == 2
                    %legend({'No obstacle', 'Top side diagonal obstacle', 'Bottom side diagonal obstacle'}, 'FontSize', 20, 'position', [0.18 0.14 0.1 0.05])
        ...
        if simulation_plot_enable == 1
            for j = 1:length(tar_freq)
                ...
                if xz_plane_plot_enable == 1
                    ...
                    %legend({'without IMN (Measured)', 'without IMN (Simulated)', 'with IMN (Measured)', 'with IMN (Simulated)'}, 'FontSize', 15, 'position', [0.4 0.4 0.12 0.06])%[0.58 0.27 0.1 0.05]);
